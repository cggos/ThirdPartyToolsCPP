...
BOOL CTestOScopeDlg::OnInitDialog()
{
  ...
  m_OScopeCtrl.SetBackgroundColor(RGB(100, 100, 100)) ;
...
{
  double nRandom,nRandom2,nRandom3;

  // generate a random number between -5 and 5
  nRandom = 5.0 + 10.0*rand()/(double)RAND_MAX/2;

  nRandom2 = abs(-5.0 + 10.0*rand()/(double)RAND_MAX);

  nRandom3 = -abs(-5.0 + 10.0*rand()/(double)RAND_MAX);

  // append the new value to the plot
  m_OScopeCtrl.AppendPoint(nRandom,nRandom2,nRandom3);
